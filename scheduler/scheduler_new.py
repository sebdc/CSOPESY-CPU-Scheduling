...
class CPUScheduler2:
    ...
    def fcfs(self):
        ...
        for p in self.processes:
            # Process details
            currentTime = max(currentTime, p.arrivalTime)   # ensure it's at least as late as the arrival time
            p.startTime = currentTime                  # process started at current time
            ...
            # Time calculation
            currentTime = p.endTime                    # update the current time
    ...
    def srtf(self):
        ...
        while self.processes or self.queue:
            ...
            if self.processes:
                self.currentTime = self.processes[0].arrivalTime if self.processes else currentTime
            ...
            # Execute the process with the shortest remaining itme 
                ...
                # Execute the process for its remaining burst time or until the next process arrives
                if self.processes:
                    nextProcessArrivalTime = self.processes[0].arrivalTime if self.processes else -1
                    runTime = min(p.burstTime, nextProcessArrivalTime+1)
                else:
                    runTime = p.burstTime

                self.currentTime = max(self.currentTime, p.arrivalTime)
                p.startTime = self.currentTime
                p.endTime = self.currentTime + runTime
                p.burstTime -= runTime
                self.currentTime += p.endTime

                print( f"{p.processId} run time = {runTime}")
                print( f"HELLO P{p.processId} with burst time {p.burstTime}")
            ...
            if p and p.burstTime > 0:
                print( f"HELLO P{p.processId} with burst time {p.burstTime}")
                print( f"ADDING P{p.processId} BACK TO QUEUE")
                self.queue.append(p)

            # Sort queue by burst time in ascending order
            self.queue.sort(key=lambda x: (x.burstTime, x.processId))
